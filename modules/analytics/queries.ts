...
export async function weeklyLeaderboard(jurisdictionId?:string,currentUserId?:string,limit=25):Promise<LeaderboardEntry[]>{
 ...
 try{
  ...
  const ids=profiles.map(profile=>profile.id),{data:ledger}=await service.from('contribution_ledger').select('citizen_id,points_delta,event_type').in('citizen_id',ids).gte('created_at',since)
  ...
  // Resolutions come from the append-only ledger, so the column agrees with the recorded
  // incident lifecycle rather than being inferred from current report status.
  const resolvedCounts=new Map<string,number>()
  for(const row of ledger||[])if(row.event_type==='REPORT_RESOLVED')resolvedCounts.set(row.citizen_id,(resolvedCounts.get(row.citizen_id)||0)+1)
  ...
  for(const entry of visible)entry.resolved=resolvedCounts.get(entry.id)||0
  return visible
 }catch{return []}
}
