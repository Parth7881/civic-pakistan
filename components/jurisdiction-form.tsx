...
export function JurisdictionForm({areas,current}:{areas:Area[];current:string|null}) {
 ...
 return <form className="form-stack" onSubmit={async event=>{event.preventDefault();setBusy(true);setError('');try{const result=await selectJurisdiction(new FormData(event.currentTarget));setError(result.error||'')}catch{setError('Your civic area could not be saved. Please try again.')}finally{setBusy(false)}}}>
