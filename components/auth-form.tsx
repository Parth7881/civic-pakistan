...
export function AuthForm({mode,portal='citizen'}:{mode:'sign-in'|'sign-up';portal?:'citizen'|'government'}) {
 ...
 return <form className="form-stack" onSubmit={submit}>
  ...
  <label>Password<PasswordInput name="password" autoComplete={mode==='sign-in'?'current-password':'new-password'} minLength={8} maxLength={128} required/></label>
  ...
  {portal==='citizen'&&<p className="muted">{mode==='sign-in'?'New here?':'Already registered?'} <Link className="text-link" href={mode==='sign-in'?'/sign-up':'/sign-in'}>{mode==='sign-in'?'Create an account':'Sign in'}</Link></p>}
 </form>
}
