...
export function AppSidebar({identity,variant}:{identity:Identity;variant:'citizen'|'government'}){
 ...
 return <aside className={`civic-sidebar${government?' is-government':''}`}>
  ...
  <div className="sidebar-city">
   <MapPin size={16}/><div><span className="sidebar-city-label">{government?'Jurisdiction':'Active city'}</span><strong>{identity.area||(government?'No assignment':'Choose your area')}</strong>{government?identity.areaCount&&identity.areaCount>1?<span className="sidebar-city-label">{identity.areaCount} areas assigned</span>:null:<Link href="/jurisdiction">Change city</Link>}</div>
  </div>
  <div className="sidebar-foot"><AccountMenu identity={identity} variant={variant} align="start"/></div>
 </aside>
}
