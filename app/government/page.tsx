...
export default async function GovernmentDashboard(){
 const {service,areas,user}=await requireGovernment()
 const [{data:items,error},global]=await Promise.all([
  service.from('incidents').select('id,jurisdiction_id,primary_report_id,status,urgency,category,created_at,sla_deadline').order('created_at',{ascending:false}).limit(500),
  governmentGlobalContext(service),
 ])
 const rows=items||[],areaMap=global.areaMap
 ...
 try{locations=await governmentGlobalIncidentLocations(service,user.id,rows.slice(0,100).map(row=>row.id))}catch{locationError=true}
 const points=rows.slice(0,100).flatMap(incident=>{
 ...
 return <>
  <section className="gov-hero">
   <HeroBackdrop/>
   <div><h1>Pakistan Civic Operations</h1></div>
   <div className="button-row">
    <Link className="secondary-button" href="/government/map"><MapPinned size={16}/> Live map</Link>
    <Link className="primary-button" href="/government/reports">Open reports <ArrowUpRight size={16}/></Link>
   </div>
  </section>
  ...
  <div className="jurisdiction-strip">
   ...
   <small>Read access covers all civic reports.</small>
  </div>

  <MetricStrip label="All reports" metrics={[
  ...
  {(error||global.error)&&<Alert tone="error">Government report data could not be loaded.</Alert>}
  {locationError&&<Alert tone="warning">Map locations need the latest Government global-read migration. Reports and status data remain available.</Alert>}
  ...
  <div className="gov-grid">
   <div className="context-stack">
    <Surface>
     ...
     {pending.length
      ?<div className="incident-list">{pending.slice(0,6).map(row=><Link className="incident-row" href={`/government/reports/${row.id}`} key={row.id} prefetch={false}>
        ...
        <div>
         <span className="caption">{areaMap.get(row.jurisdiction_id)||'Civic area'} · {formatDate(row.created_at)}</span>
    ...
    <Surface className="flush">
     ...
     <div className="gov-map">{points.length?<CivicMapFrame points={points} hrefBase="/government/reports" label="All civic incidents"/>:<div className="compact-empty"><p>No report locations are available yet.</p></div>}</div>
    </Surface>
   </div>
   <Surface>
